...
class D2STGNN(BaseModel):
    ...
    def __init__(self, model_args, **args):
        ...
        self.T_i_D_emb = nn.Parameter(
            torch.empty(model_args["tpd"], model_args["time_emb_dim"])
        )
    ...
    def forward(
        ...
        estimation_gate_feat = torch.cat(
            [
                ...
                .unsqueeze(0)
                .expand(batch_size, seq_length, -1, -1),
...
class STLocalizedConv(nn.Module):
    ...
    def forward(self, X, dynamic_graph, static_graph):
        ...
        X_k = out.transpose(-3, -2).reshape(
            batch_size, seq_len, kernel_size * node_num, num_feat
        )
...
class InhBlock(nn.Module):
    def __init__(
        self, hidden_dim, num_heads=4, bias=True, forecast_hidden_dim=256, **model_args
    ):
        super().__init__()
    ...
    def forward(self, hidden_inherent_signal):
        ...
        hidden_states_inh = hidden_states_inh.reshape(
            seq_len, batch_size, node_num, num_feat
        )
