...
class UNetMamba(BaseModel):
    ...
    def __init__(
        self, d_model, num_layers, feature, dropout=0.1, adj=None, graph_embed_dim=16,**args
    ):
        ...
        self.blocks = nn.ModuleList(
            [SimpleMambaBlock(d_model=self.d_model, dropout=self.dropout) for _ in range(self.num_layers)]
        )

        # Output projection layers
        self.time_proj = nn.Linear(self.seq_len, self.horizon)
        self.output_proj = nn.Linear(self.d_model, self.feature)
        self.quantile_context = nn.Sequential(
        ...
        self.uncertainty_proj = nn.Sequential(
            nn.LayerNorm(self.d_model),
            nn.Linear(self.d_model, self.feature),
        )
        self.scale_eps = 1e-4
    ...
    def forward(self, x):  # (B, T, N, F)
        ...
        # Inject quantile-aware context before decoding heads
        x = x + self.quantile_context(x)

        # Project feature back
        preds = self.output_proj(x)  # (B*N, H, C)
        scale = F.softplus(self.uncertainty_proj(x)) + self.scale_eps

        # Reshape back to (B, H, N, F)
        preds = preds.reshape(B, N, self.horizon, C)
        scale = scale.reshape(B, N, self.horizon, C)
        preds = preds.permute(0, 2, 1, 3)
