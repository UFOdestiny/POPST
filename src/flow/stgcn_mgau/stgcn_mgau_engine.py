...
class STGCN_mGaussian_Engine(BaseEngine):
    def __init__(self, **args):
        super(STGCN_mGaussian_Engine, self).__init__(**args)
    ...
    def evaluate(self, mode, model_path=None, export=None):
        ...
        if mode == 'val':
            ...
            for i in range(self.model.horizon):
                ...
                if pres:
                    mpiw_ = masked_mpiw_ens((preds[:, i, :], p0[:, i, :]), labels[:, i, :], mask_value, preserve=pres)

                    crps_ = masked_crps((preds[:, i, :], p0[:, i, :]), labels[:, i, :], mask_value,
                                        preserve=pres)

                else:
                    mpiw_ = masked_mpiw_ens((preds[:, i, :], p0[:, i, :]), labels[:, i, :], mask_value,
                                            preserve=pres).item()
            ...
            if mode == "all":
                ...
                print("Avg shape: ", result.shape)
                np.save(f"{self._save_path}/res_avg.npy", result)

                mae = test_mae[0].unsqueeze_(0)
                mape = test_mape[0].unsqueeze(0)
                rmse = test_rmse[0].unsqueeze(0)
