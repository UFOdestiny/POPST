...
class BTL(nn.Module):
    def __init__(self, input_dim, hid_dim, dropout=0.1):
        ...
        self.down_interact = SingleInteractModule(
            input_dim, hid_dim, input_dim, dropout
        )
...
class CM(nn.Module):
    def __init__(self, input_seq, input_dim, hid_dim, num_nodes, dropout):
        ...
        self.mixer = MixerLayer(
            self.hid_dim, 2 * self.hid_dim, self.hid_dim, self.dropout
        )
...
class ODMixer(BaseModel):
    def __init__(
        ...
        self.encoder_layer = nn.ModuleList(
            [
                ODIM(seq_len, input_dim, hid_dim, node_num, dropout)
                for _ in range(self.layer_nums)
            ]
        )
        self.trend_layer = nn.ModuleList(
            [
                BTL(self.hid_dim, self.hid_dim, self.dropout)
                for _ in range(self.layer_nums)
            ]
        )
    ...
    def forward(self, X):
        ...
        for i in range(self.layer_nums):
            ...
            _, od_feat = self.trend_layer[i](od_feat, od_feat)
