...
def generate_flow(args):
    ...
    np.savez_compressed(
        os.path.join(out_dir, "his.npz"), data=data, mean=mean, std=std, offset=offset
    )
...
def generate_od(args):
    ...
    print("save to: ", path_)
    np.savez_compressed(
        os.path.join(path_, "his.npz"), data=data
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--dataset", type=str, default="nyc_subway_od", help="dataset name"
    )
