...
ARGS="$ARGS --hidden_dim 48 --graph_dim 16 --num_layers 3 --diffusion_order 2"
ARGS="$ARGS --temporal_kernel 3 --temporal_dilation 2 --trend_kernel 3 --trend_window 6 --mlp_hidden 256"
ARGS="$ARGS --dropout 0.2 --lrate 0.001 --wdecay 1e-4 --clip_grad_norm 5"

echo "=== Running fmgcn4 ==="
python3 $SRC/main.py $ARGS 2>&1 | tee $LOG/fmgcn4.log
echo "fmgcn4 finished with exit code $?"
